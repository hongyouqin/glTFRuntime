...
UStaticMesh* FglTFRuntimeParser::LoadStaticMesh_Internal(TArray<TSharedRef<FJsonObject>> JsonMeshObjects, const FglTFRuntimeStaticMeshConfig& StaticMeshConfig, const TMap<TSharedRef<FJsonObject>, TArray<FglTFRuntimePrimitive>>& PrimitivesCache)
{
	...
	for (TSharedRef<FJsonObject> JsonMeshObject : JsonMeshObjects)
	{
		...
		StaticMesh->GetStaticMaterials().Append(StaticMaterials);

		
		if (bCalculateNormals || bCalculateTangents)
		{
			
			FStaticMeshOperations::ComputeTriangleTangentsAndNormals(MeshDescription->GetMeshDescription());
			...
			FStaticMeshOperations::RecomputeNormalsAndTangentsIfNeeded(MeshDescription->GetMeshDescription(), NTPBsFlags);
		}
	...
	if ((bHasVertexColors || bIsMobile) && StaticMesh->GetRenderData() && StaticMesh->GetRenderData()->LODResources.Num() > 0)
	{
		StaticMesh->ReleaseResources();
		for (int32 LODIndex = 0; LODIndex < StaticMesh->GetRenderData()->LODResources.Num(); LODIndex++)
		{
			StaticMesh->GetRenderData()->LODResources[LODIndex].bHasColorVertexData = true;
		}
	...
	for (const TPair<int32, float>& Pair : StaticMeshConfig.LODScreenSize)
	{
		...
		if (StaticMesh->GetRenderData() && CurrentLODIndex >= 0 && CurrentLODIndex < StaticMesh->GetRenderData()->LODResources.Num())
		{
			StaticMesh->GetRenderData()->ScreenSize[CurrentLODIndex].Default = Pair.Value;
		}
	}
	...
	StaticMesh->GetBodySetup()->bMeshCollideAll = false;
	StaticMesh->GetBodySetup()->CollisionTraceFlag = StaticMeshConfig.CollisionComplexity;
	...
	for (const FVector4 Sphere : StaticMeshConfig.SphereCollisions)
	{
		...
		StaticMesh->GetBodySetup()->AggGeom.SphereElems.Add(SphereElem);
	}
